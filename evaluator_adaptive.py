"""
Адаптивный evaluator - автоматически выбирает между обычным и vLLM evaluator.
"""
import os
import sys

# Проверяем переменные окружения
USE_VLLM = os.environ.get("USE_VLLM_EVALUATOR", "").lower() in ("true", "1", "yes")
VLLM_API_BASE = os.environ.get("VLLM_API_BASE", "http://127.0.0.1:8000/v1")

if USE_VLLM:
    print(f"[evaluator-adaptive] Используем vLLM evaluator ({VLLM_API_BASE})")
    try:
        # Пробуем импортировать vLLM evaluator
        from evaluator_vllm import evaluate, vLLMEvaluator
        print("[evaluator-adaptive] ✓ vLLM evaluator загружен успешно")
    except ImportError as e:
        print(f"[evaluator-adaptive] ✗ Не удалось загрузить vLLM evaluator: {e}")
        print("[evaluator-adaptive] Переключаемся на обычный evaluator")
        USE_VLLM = False
    except Exception as e:
        print(f"[evaluator-adaptive] ✗ Ошибка при загрузке vLLM: {e}")
        print("[evaluator-adaptive] Переключаемся на обычный evaluator")
        USE_VLLM = False

if not USE_VLLM:
    print("[evaluator-adaptive] Используем обычный evaluator (transformers)")
    # Динамически создаем обычный evaluator на основе исходного кода
    ...
    def _load_model():
        ...
        model = AutoModelForCausalLM.from_pretrained(
            BASE_MODEL, device_map=DEVICE, torch_dtype=torch.float16, trust_remote_code=True
        )
        ...
        if not os.path.exists(os.path.join(PATH_ADAPTER, "adapter_config.json")):
            ...
            extracted_root = os.path.join(
                ...
                "checkpoints", "126_csdn_disQwen0.5B"
            )
    ...
    def guess_one(
        ...
        num_guesses,
        batch_size,
        extra_guesses_ratio: float
    ):
        """Оригинальная функция guess_one из основного evaluator."""
        ...
        inputs = tokenizer(
            full_input, return_tensors="pt", truncation=True, max_length=max_length_sysprompt
        ).to(model.device)
        ...
        def _collect_guesses(out_tensor, required_length=None):
            for i in range(out_tensor.shape[0]):
                generated = tokenizer.decode(
                    out_tensor[i][inputs["input_ids"].shape[1]:], skip_special_tokens=True
                )
    ...
    def evaluate(program_path: str) -> Dict[str, Any]:
        """Оценивает качество промпта на train.json."""
        started_at = time.time()
        dump_enabled = bool(os.environ.get("EnableDump"))
        dump_preview = int(os.environ.get("PASSLLM_EVAL_DUMP_PREVIEW_COUNT", "999999999"))
        dump_dir = os.environ.get("PASSLLM_EVAL_DUMP_PATH", "./logs")
        ...
        try:
            ...
            raise ValueError(f"Error reading prompt: {e}")
        
        prompt_length = len(prompt_text)
        
        if not PATH_TRAIN or not os.path.exists(PATH_TRAIN):
            raise ValueError("train.json not found")
        ...
        print(f"[evaluate] to_check={len(train_data)} (max_check_example={NUM_GUESSES})")
...
print("[evaluator-adaptive] Инициализация завершена")
